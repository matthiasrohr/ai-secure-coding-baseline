...
- **Proven Mechanisms:** Reuse the project's established, sound mechanisms and actively maintained libraries — flag a weak one, do not silently replace it; never hand-roll crypto, auth, or session handling; use vetted algorithms and a CSPRNG — no MD5/SHA-1 for security, no `Math.random` for tokens, hash passwords with a slow KDF (argon2/bcrypt).
